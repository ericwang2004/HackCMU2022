...
class Camera:
    def __init__(self, tile_camera_height, tile_camera_width, tile_pixel_dimensions, cameraX, cameraY) -> None:
        self.tile_camera_height = tile_camera_height
        self.tile_camera_width = tile_camera_width
        self.tile_pixel_dimensions = tile_pixel_dimensions
        self.pos = Vector(cameraX, cameraY)
    ...
    def to_pixel_cord(self, pos):
        ...
        #if pos.x <= self.pos.x - 1 or pos.x >= self.pos.x + self.tile_camera_width or pos.y <= self.pos.y - 1 or pos.y >= self.pos.y + self.tile_camera_height:
         #   return None
        
        #relative cord to the camera's position
        relative_cord = pos.sub_new(self.pos) 
        #proportion cord from range [0, 1]
        proportion_cord = Vector(relative_cord.x / self.tile_camera_width, relative_cord.y / self.tile_camera_height)
